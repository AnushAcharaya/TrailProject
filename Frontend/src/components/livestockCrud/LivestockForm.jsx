  ...
  if (loading) {
    return (
      <div className="livestock-form-card">
        <p className="text-center text-gray-600">{t('form.messages.loading')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="livestock-form-card">
      <h2 className="text-2xl font-semibold mb-6 text-green-700">
        {isEditing ? t('form.title.edit') : t('form.title.add')}
      </h2>

      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Tag ID - Auto-generated, Read-only */}
        <div className="flex items-center space-x-3">
          <label className="block text-sm font-medium w-24">{t('form.fields.tagId')}</label>
          <input
            type="text"
            name="tag_id"
            value={formData.tag_id || t('form.placeholders.autoGenerated')}
            readOnly
            disabled
            className="flex-1 border rounded-lg p-2 bg-gray-100 text-gray-600 cursor-not-allowed"
            placeholder={t('form.placeholders.tagId')}
          />
        </div>

        {/* Species - Searchable Dropdown */}
        <div className="flex items-center space-x-3 relative">
          <label className="block text-sm font-medium w-24">{t('form.fields.species')}</label>
          <div className="flex-1 relative">
            <div className="relative">
              <input
                ...
                placeholder={t('form.placeholders.species')}
                required
                ...
                {speciesList.length > 0 ? (
                  filteredSpecies.length > 0 ? (
                    ...
                    <div className="px-4 py-2 text-gray-500 text-center">
                      {t('form.messages.noSpecies', { query: speciesInput })}
                    </div>
                  )
                ) : (
                  <div className="px-4 py-2 text-gray-500 text-center">
                    {t('form.messages.loadingSpecies')}
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex items-center space-x-3 relative">
          <label className="block text-sm font-medium w-24">{t('form.fields.breed')}</label>
          <div className="flex-1 relative">
            <div className="relative">
              <input
                type="text"
                ...
                placeholder={formData.species ? t('form.placeholders.breed') : t('form.placeholders.breedDisabled')}
                required
              ...
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {breedsList.length > 0 ? (
                  filteredBreeds.length > 0 ? (
                    ...
                    <div className="px-4 py-2 text-gray-500 text-center">
                      {t('form.messages.noBreeds', { query: breedInput })}
                    </div>
                  )
                ) : (
                  <div className="px-4 py-2 text-gray-500 text-center">
                    {t('form.messages.loadingBreeds')}
                  </div>
                )}
              </div>
            )}
        ...
        {/* Date of Birth */}
        <div className="flex items-center space-x-3">
          <label className="block text-sm font-medium w-24">{t('form.fields.dob')}</label>
          <input
            type="date"
            name="date_of_birth"
        ...
        {/* Gender */}
        <div className="flex items-center space-x-3">
          <label className="block text-sm font-medium w-24">{t('form.fields.gender')}</label>
          <select
            name="gender"
        ...
        {/* Color */}
        <div className="flex items-center space-x-3">
          <label className="block text-sm font-medium w-24">{t('form.fields.color')}</label>
          <input
            type="text"
            name="color"
        ...
        {/* Health Status */}
        <div className="flex items-center space-x-3">
          <label className="block text-sm font-medium w-24">{t('form.fields.health')}</label>
          <select
            name="health_status"
            ...
            className="flex-1 border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-green-500 hover:border-green-400 transition"
          >
            <option value="">{t('form.placeholders.selectHealth')}</option>
            <option value="Healthy">{t('form.health.healthy')}</option>
            <option value="Under Treatment">{t('form.health.underTreatment')}</option>
            <option value="Critical">{t('form.health.critical')}</option>
            <option value="Deceased">{t('form.health.deceased')}</option>
          </select>
        </div>

        {/* Image Upload */}
        <div className="md:col-span-2">
          <label className="block text-sm font-medium mb-2">{t('form.fields.image')}</label>
          <div
          ...
          >
            {formData.imagePreview ? (
              <div>
                ...
                />
                <p className="mt-2 text-xs text-gray-500">{t('form.placeholders.imageChange')}</p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                <FaImage className="text-4xl text-gray-400 mb-2" />
                <p className="text-sm text-gray-600">{t('form.placeholders.imageUpload')}</p>
                <p className="text-xs text-gray-400">{t('form.placeholders.imageFormat')}</p>
              </div>
            )}
            <input
              id="image-upload"
        ...
        {/* Remarks */}
        <div className="md:col-span-2">
          <label className="block text-sm font-medium">{t('form.fields.remarks')}</label>
          <textarea
            name="remarks"
        ...
        <div className="md:col-span-2 flex justify-between items-center gap-4">
          ...
          >
            {isEditing ? t('form.buttons.update') : t('form.buttons.submit')}
          </button>
        </div>
      </form>
    </div>
